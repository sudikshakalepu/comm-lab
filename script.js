/*
 * - Integrated YouTube IFrame API for programmatic video control (play/pause/seek)
 * - Made all timing dynamic based on actual video duration instead of hardcoded values
 * - Results popup triggered by video end event OR fallback timeout
 * - All playback controls (quiz submit, rewatch, skip, retake) now use YouTube API
 * - User can pause/play video manually while maintaining auto-playback functionality
 * - Video end listener enables/disables appropriately to prevent duplicate results
 */
...
let viewerCount       = 769; // 09/02: starts at 769, climbs to 1043 over video duration
let resultsTimeout    = null;

// 09/02: YouTube player instance and video duration tracking for dynamic timing
let player = null;
let videoDuration = 30; // fallback duration in seconds
let videoEndListener = null;

// 09/02: Initialize YouTube player when API is ready
...
function onPlayerReady(event) {
    // 09/02: Get actual video duration once player is ready
    videoDuration = player.getDuration();
    console.log('Video duration:', videoDuration, 'seconds');
}

function onPlayerStateChange(event) {
    // 09/02: When video ends (state 0), show results
    if (event.data === YT.PlayerState.ENDED && videoEndListener) {
        showResults();
    }
}
...
quizForm.addEventListener('submit', function(e) {
    ...
    // tally scores (radio names are q3, q4, q5)
    ...
    // hide quiz, show "watch the film" message
    quizPopup.classList.add('hidden');
    videoMessagePopup.classList.remove('hidden');

    // after 2s: hide message, autoplay video, show IG overlay + skip button
    setTimeout(function() {
        ...
        startIGLiveOverlay();
        videoEndListener = true;

        // show floating skip button
...
function spawnComicShape() {
    ...
    switch (zone) {
        ...
        case 2: // top right
            ...
            break;
        case 3: // bottom right (above YouTube controls)
            el.style.right = (6 + Math.random() * 9) + '%';
...
// 09/02: viewer count climbs from 769 to 1043 over the actual video duration
function startViewerCount() {
    const START   = 769;
    const END     = 1043;
    const TOTAL_MS = videoDuration * 1000; // 09/02: use actual video duration
...
// skip to results (floating button)
...
// 09/02: auto rewatch on load (from cast page) - waits for player to be ready
(function() {
    if (sessionStorage.getItem('rewatch') === '1') {
        sessionStorage.removeItem('rewatch');

        // 09/02: wait for YouTube player to be ready before triggering rewatch
        const tryRewatch = setInterval(function() {
            if (player && player.playVideo) {
                clearInterval(tryRewatch);
